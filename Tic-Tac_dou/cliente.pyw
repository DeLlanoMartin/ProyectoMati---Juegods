...
jugador_label = customtkinter.CTkLabel(app, text="Esperando ID...", font=("Arial", 18))
jugador_label.grid(row=0, column=0, columnspan=7, pady=10, sticky="n")

message_label = customtkinter.CTkLabel(app, text="Esperando inicio...", font=("Arial", 16))
...
def receive_messages():
    ...
    color = "red" if client_id == 1 else "blue"
    ...
    while True:
        try:
            ...
            if player == "1":
                buttons[button_index].configure(text="X", state="disabled", fg_color="red", font=("Arial", 30))
            elif player == "2":
                buttons[button_index].configure(text="O", state="disabled", fg_color="blue", font=("Arial", 30))
            ...
            if client_id == turn and not game_over:
                message_label.configure(text="Tu turno")
                enable_buttons()
            else:
                message_label.configure(text="Esperando tu turno...")
                disable_buttons()
...
def handle_game_over(winner_id):
    ...
    # Actualizar el mensaje que indica el ganador
    message_label.configure(text=f"¡El jugador {winner_id} ha ganado!")
...
def button_callback(i):
    ...
    if not game_over and board[i-1] is None:  # Solo permitir si no hay ganador
        if client_id == 1:
            buttons[i-1].configure(text="X", state="disabled", fg_color="red", font=("Arial", 30))  # Jugador 1 usa "X"
            board[i-1] = "X"
        elif client_id == 2:
            buttons[i-1].configure(text="O", state="disabled", fg_color="blue", font=("Arial", 30))  # Jugador 2 usa "O"
            board[i-1] = "O"
